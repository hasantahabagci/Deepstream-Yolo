...
FPS = 40
APP_CONFIG_PATH = "/home/ituarc/DeepStream-Yolo/deepstream_app_config.txt"
...
def enhance_small_target_rgba_inplace(rgba_frame):
    """
...
def preprocess_sink_pad_buffer_probe(pad, info, u_data):
    global ENABLE_SMALL_TARGET_PREPROCESS, preprocess_warning_printed

    if shutdown_event.is_set() or not ENABLE_SMALL_TARGET_PREPROCESS:
    ...
    while l_frame is not None:
        ...
        if PREPROCESS_INTERVAL > 1 and frame_meta.frame_num % PREPROCESS_INTERVAL != 0:
            try:
        ...
        try:
            ...
            enhance_small_target_rgba_inplace(rgba_frame)
        except Exception as exc:
            if not preprocess_warning_printed:
                print(f"Small-target preprocess disabled after error: {exc}")
...
def create_pipeline():
    ...
    if ENABLE_SMALL_TARGET_PREPROCESS and not all([preprocess_convert, preprocess_caps]):
    ...
    pgie.set_property("config-file-path", "/home/ituarc/DeepStream-Yolo/config_infer_primary_yolo26.txt")
