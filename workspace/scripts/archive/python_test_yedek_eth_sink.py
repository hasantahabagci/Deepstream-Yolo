...
def create_pipeline():
    """
    Create a DeepStream pipeline with an added RTP streaming sink.
    """
    ...
    # --- New elements for RTP streaming ---
    queue3 = Gst.ElementFactory.make("queue", "queue3")
    rtp_encoder = Gst.ElementFactory.make("nvv4l2h264enc", "rtp-encoder")
    rtp_payload = Gst.ElementFactory.make("rtph264pay", "rtp-payload")
    ...
    if not source or not streammux or not pgie or not nvvidconv or not nvosd:
        sys.stderr.write("Unable to create base elements\n")
        sys.exit(1)
    ...
    # --- Check if new RTP elements were created ---
    if not queue3 or not rtp_encoder or not rtp_payload or not udp_sink:
        sys.stderr.write("Unable to create RTP streaming elements\n")
        sys.exit(1)

    # --- Set properties for existing elements ---
    ...
    # --- Configure the new RTP elements ---
    print("Configuring RTP stream sink...")
    rtp_encoder.set_property("bitrate", 4000000)
    rtp_encoder.set_property("preset-level", 1) # Using a low-latency preset
    rtp_encoder.set_property("insert-sps-pps", True) # Crucial for the receiver to decode
    
    # IMPORTANT: Change this to the IP address of the receiving computer
    udp_sink.set_property("host", "10.42.0.160") 
    ...
    # Add new RTP elements
    pipeline.add(queue3)
    pipeline.add(rtp_encoder)
    pipeline.add(rtp_payload)
    ...
    # --- Link the new RTP streaming branch ---
    tee.link(queue3)
    queue3.link(rtp_encoder)
    rtp_encoder.link(rtp_payload)
    rtp_payload.link(udp_sink)
